...
/**
 * A small bar under the selection for the things you may keep: pin them so
 * they never age, or make them fresh again right now. Shows for the desktop's
...
export function SelectionActions({ editor, me, owner, metas, metaVersion, sync }: { editor: Editor; me: Me; owner: string | null; metas: Map<string, ItemMeta>; metaVersion: number; sync: BoardSync | null }) {
  ...
  return (
    <div className="SelectionActions" style={{ transform: `translate(${state.x}px, ${state.y}px) translate(-50%, 10px)` }} onPointerDown={(e) => e.stopPropagation()}>
      <button type="button" className={`SelectionActions-button${state.allPinned ? ' SelectionActions-button--on' : ''}`} onClick={() => sync?.pin(state.ids, !state.allPinned)} title={state.allPinned ? 'Let it age again' : 'Never let this fade'}>
        <PinIcon /> {state.allPinned ? 'Pinned' : 'Pin'}
        {n > 1 ? ` ${n}` : ''}
      </button>
      {state.anyStale && (
